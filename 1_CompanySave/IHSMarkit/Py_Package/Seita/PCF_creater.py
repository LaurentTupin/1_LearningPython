...
def producePCF(str_pcf, str_folderRoot, dte_date, bl_ArchiveMails = False):
    ...
    try:
        str_resultFigures, dic_df = dwl.fDic_downloadFiles(str_folderRoot, dte_date, bl_ArchiveMails, str_pcf, True, False)
...
#-----------------------------------------------------------------
# Create PCF Files (+ csv)
#-----------------------------------------------------------------
def pcfCreate_createFiles(str_PCF, str_folderRoot, dte_date, dic_df):
    try:
        # Get Params
        df_Param = pcfCreate_GetParams(str_PCF)
        # Create the PCF
        d_path = pcfCreate_PCF(df_Param, dic_df, str_folderRoot, dte_date)
        # Remove str_folderRoot to the path
        l_pathAttach = list(d_path.values())
        l_pathAttach = [path.replace(str_folderRoot, '') for path in l_pathAttach]
    except:		return []
    return l_pathAttach

def pcfCreate_GetParams(str_PCF):
    try:
        df_Param = pd.read_csv('Seita_Param_AdrPcf.csv')
        df_Param = df_Param.loc[df_Param['PCF'] == str_PCF]
        df_Param.fillna('', inplace = True)
        if df_Param.empty:          
            print('  (*) df_Param is empty')
            raise
    except Exception as err:
        print(' ERROR pcfCreate_GetParams: Cannot read Seita_Param_AdrPcf.csv || '.format(err))
        return []
    return df_Param

def pcfCreate_PCF(df_Param, dic_df, str_folderRoot, dte_date):
    d_fileType = {'csv': ['.csv', '.txt', '.tmp', '.etf'], 'xls': ['.xls', 'xlsx', 'xlsm']}
    d_path = {}
    d_options = {}
        
    for i, row in enumerate(df_Param.index):
        # Get the paths / other params
        try:
            str_ID = str(df_Param.loc[row, 'ID'])
            str_folder = pp.fStr_generateFolderRaw(df_Param, row, str_folderRoot, dte_date)
            str_fileName = pp.fStr_generateFileName(df_Param, row, str_folder, dte_date, False)
            str_path = os.path.join(str_folder, str_fileName)
            d_path[str_ID] = str_path
            # other options
            str_separator = str(df_Param.loc[row, 'Separator'])
            str_SheetName = str(df_Param.loc[row, 'SheetName'])
            if str(df_Param.loc[row, 'bl_header']).lower() == 'true':   bl_header = True
            else:                                                       bl_header = False
            str_options = str(df_Param.loc[row, 'Excel_options'])
            l_options = str_options.split(',')
            for options in l_options:
                d_options[options] = True
            str_format = str(df_Param.loc[row, 'FORMAT'])
        except Exception as err:
            print(' ERROR pcfCreate_PCF: Cannot generate Paths ||| '.format(err))
            raise
        # Get the Data
        try:	df_data = dic_df[str_ID]
        except:	
            print(' ERROR in pcfCreate_PCF: str_ID doesnt exist in the dic of df: {}  ||| {}'.format(str_ID, dic_df))
            raise
        # Create the files
        try:
            if isinstance(df_data, str):
                if 'no file to create' in df_data.lower() or 'return the path' in df_data.lower():
                    print('  (*) PCF just to be forwarded || {}...  || {}'.format(df_data, fl.fStr_GetFileFromPath(str_path)))
                else:
                    print('  (*) Is that normal ???? || {}...  || {}'.format(df_data, fl.fStr_GetFileFromPath(str_path)))
            elif str_path[-4:] in d_fileType['csv']:
                fl.fStr_CreateTxtFile(str_path, '', df_data, str_sep = str_separator)
            elif str_path[-4:] in d_fileType['xls']:
                if isinstance(df_data, dict): 		#if type(df_data) is dict :
                    l_sheetName = list(df_data.keys())
                    l_df_data = list(df_data.values())
                    fl.fStr_createExcel_SevSh(str_path, '', l_df_data, l_sheetName, bl_header = bl_header, d_options = d_options)
                    # Save a CSV Copy
                    for key_sheetName, value_df in df_data.items():
                        pcfCreate_PCFonCsv(str_path, value_df, key_sheetName)
                        fl.Act_StyleIntoExcel(str_path, str_format, key_sheetName)
                else:
                    fl.fStr_createExcel_SevSh(str_path, '', [df_data], [str_SheetName], bl_header = bl_header, d_options = d_options)
                    # Save a CSV Copy
                    pcfCreate_PCFonCsv(str_path, df_data)
                    fl.Act_StyleIntoExcel(str_path, str_format, '')
            else:
                print(' ERROR: the kind of files is not defined ||| {}'.format(str_path))
                raise
        except Exception as err:
            print(' ERROR pcfCreate_PCF')
            print(' - Err :', err)
            return {}
    return d_path

def pcfCreate_PCFonCsv(str_path, df_data, str_sheetName = ''):
    try:
        # Redefine Folder and filename
        str_fileName = fl.fStr_GetFileFromPath(str_path)
        if str_sheetName == '':
            str_fileName = '{}.csv'.format(str_fileName)
        else:
            str_fileName = '{}_{}.csv'.format(str_fileName, str_sheetName)
        str_folder = fl.fStr_GetFolderFromPath(str_path)
        str_folder = fl.fStr_BuildPath(str_folder, 'csv_copy')
        # Create the folder
        fl.fBl_createDir(str_folder)
        # Create Files
        fl.fStr_CreateTxtFile(str_folder, str_fileName, df_data)
    except Exception as err:
        print(' ERROR pcfCreate_PCFonCsv')
        print(' - Err :', err)
        return False
    return True
...
def pcf_Nikko63(str_PCF, str_folderRoot, dte_date, str_resultFigures, dic_df):
    ...
    # 3. Build the return message
    try:
        str_resultFigures += '\n' + str(round(flt_Nav, 4)) + ' : Nav per Unit'
        str_resultFigures += '\n' + str(round(flt_aum, 0)) + ' : Asset Under M'
        str_resultFigures += '\n' + str(round(flt_share, 0)) + ' : Total Shares in Issue'
    except Exception as err:    return 'ERROR: 3. Build the return message - {} | {}'.format(str_PCF, str(err)), []
    
    # 4. Build Header from Model
    try:
        # Model Path
...
#                             'Nikko AM Global Internet ETF - SGX - ' + dte_navDate.strftime('%Y%m%d') + '.xlsx']
        ...
        df_header = pd.read_excel(fl.fStr_BuildPath(str_modelfolder, str_modelFileName), header = None, index_col = None)
        df_header.fillna(value = '', inplace = True)
        df_header.iloc[0, 1] = dte_navDate.strftime('%d-%b-%y')
        df_header.iloc[5, 1] = str(flt_Nav)
        df_header.iloc[6, 1] = str(flt_aum)
        df_header.iloc[7, 1] = str(flt_share)
        # Tail: Disclaimer
        int_ImportantNotice = int(df_header.loc[df_header[0] == 'Important Notice:'].index.values)
        df_tail = df_header.iloc[int_ImportantNotice:]      
        # SGX
        df_headerSGX = pd.read_excel(fl.fStr_BuildPath(str_modelfolder, str_modelFileName_SGX), header = None, index_col = None)
        df_headerSGX.fillna(value = '', inplace = True)
        df_headerSGX.iloc[3, 2] = dte_navDate.strftime('%d-%b-%y')
        df_headerSGX.iloc[11, 2] = str(flt_Nav)
        df_headerSGX.iloc[12, 2] = str(flt_aum)
        df_headerSGX.iloc[13, 2] = str(flt_share)
        df_headerSGX.iloc[14, 2] = str(int_CreationUnits)
        df_headerSGX.iloc[15, 2] = str(flt_BasketCash)
        # CONCAT
        df_BasketFinal = dframe.fDf_Concat_wColOfDf1(df_header.loc[:9], df_Basket)
        df_BasketFinal = dframe.fDf_Concat_wColOfDf1(df_BasketFinal, df_tail, int_emptyRow = 1)
        df_FundFinal = dframe.fDf_Concat_wColOfDf1(df_header.loc[:9], df_Fund)
        df_FundFinal = dframe.fDf_Concat_wColOfDf1(df_FundFinal, df_tail, int_emptyRow = 1)
        # Exception for 63
        if '63' in str_PCF:
            df_FundFinal.iloc[6, 0] = 'Asset Under Management (Total fund)'
            df_FundFinal.iloc[6, 1] = str(flt_aum_share)        
        df_SGXfinal =  dframe.fDf_Concat_wColOfDf1(df_headerSGX.loc[:18], df_SGX)
    except Exception as err:    return 'ERROR: 4. Build Header from Model - {} | {}'.format(str_PCF, str(err)), []

    # 4. Create the files
    dic_df['pcf_Basket'] =  df_BasketFinal
    dic_df['pcf_Fund'] =    df_FundFinal
    dic_df['pcf_SGX'] =     df_SGXfinal
    l_pathAttach = pcfCreate_createFiles(str_PCF, str_folderRoot, dte_date, dic_df)
...
def pcf_EasyComo(str_PCF, str_folderRoot, dte_date, str_resultFigures, dic_df):
    ...
    except Exception as err: return 'ERROR: Build Final DF - {} | {}'.format(str_PCF, str(err)), []
    ...
    try:
        ...
        # CONCAT
        df_targetNav = dframe.fDf_Concat_wColOfDf1(df_MACRO_NAV, df_NavToAdd)
        df_targetNav.fillna(value = '', inplace = True)
    except Exception as err: return 'ERROR: Build the Target Nav File - {} | {}'.format(str_PCF, str(err)), []
    
    # Create the files
    try:
        dic_df['pcf_CO_GSCU'] =     df_finalGSCU
        dic_df['pcf_CO_GSCE'] =     df_finalGSDE
        dic_df['pcf_PCF_GSCU'] =    df_finalPCFGSCU
        dic_df['pcf_PCF_GSCE'] =    df_finalPCFGSCE
        dic_df['pcf_targetNav'] =   df_targetNav
        l_pathAttach = pcfCreate_createFiles(str_PCF, str_folderRoot, dte_date, dic_df)
    except Exception as err: return 'ERROR: Create the files - {} | {}'.format(str_PCF, str(err)), []
...
def pcf_WisdomTree(str_PCF, str_folderRoot, dte_date, str_resultFigures, dic_df):
    ...
    # Build ETF DF
    try:
        # Get the NAV from WCOA and WCOG
        flt_navWCOA = df_WCOA.iloc[1, 11]
        flt_navWCOG = df_WCOG.iloc[1, 11]
        # Build ETF
        df_Wcom_etf = pp.fDf_WtEtfFile(dte_date, 'DE000A2X2PA9', '0YS1', 'IE00BG88WH84', 'GBP', flt_navWCOM)
        df_Wcoe_etf = pp.fDf_WtEtfFile(dte_date, 'DE000A2NC9W9', '0EXC', 'IE00BG88WG77', 'EUR', flt_navWCOE)
        df_Coms_etf = pp.fDf_WtEtfFile(dte_date, 'DE000A21VY91', '3PWC', 'IE00BG88WL21', 'CHF', flt_navCOMS)
        df_WCOA_etf = pp.fDf_WtEtfFile(dte_date, 'DE000A169VT9', '3XL7', 'IE00BYMLZY74', 'USD', flt_navWCOA)
        df_WCOG_etf = pp.fDf_WtEtfFile(dte_date, 'DE000A2X2N83', '0YQZ', 'IE00BZ1GHD37', 'USD', flt_navWCOG)
    except Exception as err: return 'ERROR: Build ETF DF - {} | {}'.format(str_PCF, str(err)), []
    ...
    # Create the files
    try:
        dic_df['pcf_Wcom'] = df_Wcom
        dic_df['pcf_Wcoe'] = df_Wcoe
        dic_df['pcf_Coms'] = df_Coms
        dic_df['pcf_WCOA'] = 'No file to create, return the path'
        dic_df['pcf_WCOG'] = 'No file to create, return the path'
        dic_df['pcf_Wcom_etf'] = df_Wcom_etf
        dic_df['pcf_Wcoe_etf'] = df_Wcoe_etf
        dic_df['pcf_Coms_etf'] = df_Coms_etf
        dic_df['pcf_WCOA_etf'] = df_WCOA_etf
        dic_df['pcf_WCOG_etf'] = df_WCOG_etf
        l_pathAttach = pcfCreate_createFiles(str_PCF, str_folderRoot, dte_date, dic_df)
...
def pcf_ChinaAMC(str_PCF, str_folderRoot, dte_date, str_resultFigures, dic_df):
    ...
    try:
        fl.fStr_fillXls_df_xlWgs(str_modelfolder + str_modelFileName, df_Result)
    except: return 'ERROR: {} - Feeder le model pour avoir le dernier RIC du Fut'.format(str_PCF), []
    ...
    # Create the files
    try:
        dic_df['pcf_Lev_2x'] =  df_Lev_2x
        dic_df['pcf_Lev_m1x'] = df_Lev_m1x
        dic_df['pcf_Lev_m2x'] = df_Lev_m2x
        l_pathAttach = pcfCreate_createFiles(str_PCF, str_folderRoot, dte_date, dic_df)
        #        # END
        #        str_folder = dic_df['Folder']
        ...
        #        l_pathAttach = [str_path0, str_path1, str_path2]
        #        l_pathAttach = [path.replace(str_folderRoot, '') for path in l_pathAttach]
    except Exception as err: return 'ERROR: Create the files || {0} || {1}'.format(str_PCF, str(err)), []
        
    # Close Excel
    try:
        inst_xlWings = fl.c_xlApp_xlwings()
        inst_xlWings.Quit_xlApp(bl_force = False, bl_killExcelProcess = False)
    except Exception as err: return 'ERROR: Close Excel || {0} || {1}'.format(str_PCF, str(err)), []
...
def pcf_GlobalX(str_PCF, str_folderRoot, dte_date, str_resultFigures, dic_df):
    ...
    try:
        ...
        str_newPath = str_pcfpath.replace('\\raw','')
        shutil.copyfile(str_pcfpath, str_newPath)
        # Rename
        str_newFolder = fl.fStr_GetFolderFromPath(str_newPath)
        str_oldName = fl.fStr_GetFileFromPath(str_pcfpath)
        str_newName = 'PCF_{}(Incl FX).xlsx'.format(dte_date.strftime('%Y%m%d'))
    ...
    try:
        ...
        df_FDSAITR_head, dte_FDSAITR = fDf_GetHeader_forICE(str_pcfpath, 'FDSAITR', None, 2807, 'HK0000637857')
        df_FDSSMTR_head, dte_FDSSMTR = fDf_GetHeader_forICE(str_pcfpath, 'FDSSMTR', None, 3191, 'HK0000637832')
    ...
    try:
        ...
        df_FDSAITR = fDf_GetCompo_forICE(str_pcfpath, 'FDSAITR', 12, dte_FDSAITR, 2807, 'HK0000637857')
        df_FDSSMTR = fDf_GetCompo_forICE(str_pcfpath, 'FDSSMTR', 12, dte_FDSSMTR, 3191, 'HK0000637832')
    ...
    try:
        ...
        df_FDSAITR_head.iloc[0, 4] = len(df_FDSAITR.index)
        df_FDSAITR_head.iloc[0, 5] = 'CNH'
        df_FDSAITR_head.iloc[0, 10] = '50000'
        df_FDSSMTR_head.iloc[0, 4] = len(df_FDSSMTR.index)
        df_FDSSMTR_head.iloc[0, 5] = 'CNH'
        df_FDSSMTR_head.iloc[0, 10] = '50000'
    ...
    try:
        ...
        # PCF as of August 2020
        df_ICE = dframe.fDf_Concat_wColOfDf1(df_ICE, df_FDSAITR_head)
        df_ICE = dframe.fDf_Concat_wColOfDf1(df_ICE, df_FDSAITR)
        df_ICE = dframe.fDf_Concat_wColOfDf1(df_ICE, df_FDSSMTR_head)
        df_ICE = dframe.fDf_Concat_wColOfDf1(df_ICE, df_FDSSMTR)
    except Exception as err: return 'ERROR: Merge the Dataframe - {0} | {1} '.format(str_PCF, str(err)), []
    
    # Create the files
    try:
        dic_df['pcf_PCF'] =     'No file to create, return the path'
        dic_df['pcf_ICE'] =     df_ICE
        dic_df['pcf_ICE_tmp'] = df_ICE
        l_pathAttach = pcfCreate_createFiles(str_PCF, str_folderRoot, dte_date, dic_df)
        #        # PATH
        #        str_folder = dic_df['Folder']
        #        l_pcfFileName = ['PCF_{}(Incl FX).xlsx'.format(dte_date.strftime('%Y%m%d')), 
        #                         'MiraeAsset.{}.csv'.format(dte_date.strftime('%Y%m%d'))]
        #        l_pathAttach = [str_folder + l_pcfFileName[0]]
        #        # Create ICE
